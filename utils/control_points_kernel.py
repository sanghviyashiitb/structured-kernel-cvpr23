...
def convert_trajectory_to_kernels(trajectory_3d, grid_in, K_list):
	...
	Tx_arr = np.linspace(-64,64,100)
	Ty_arr = np.linspace(-64,64,100)
...
def get_control_points(max_control_points, max_speed = 32):
	N_c = np.random.randint(2,max_control_points+1)
	control_points = np.zeros([N_c, 2], dtype=np.float32)
	curr_pos = np.zeros([2])
	for idx in range(1, N_c):
		theta0, rho = np.random.uniform()*360, max_speed*np.random.uniform()  
...
def create_trajectory(control_points, N, center=True):
	x = control_points[:,0]
	y = control_points[:,1]	
	k_hat = min(2, len(x)-1)
...
def control_points_to_kernel(control_points, k_size=64, center=True):
	F_pixels = 1
	k_size = k_size
	...
	trajectory = create_trajectory(control_points, 500, center)
	kernel = convert_trajectory_to_kernels(trajectory, grid_in, K_list)
	if center:
...
# z[1,0] = -9.741721; z[1,1] = 11.40609   
# z[2,0] = -21.14781; z[2,1] = 1.6643691
# z[3,0] = 1.2441155; z[3,1] = 5.2959737
# trajectory = create_trajectory(z, 500)
