...
def convert_trajectory_to_kernels(trajectory_3d, grid_in, K_list):
	...
	trajectory_3d = np.clip(trajectory_3d, -IM_SIZE/2.0, IM_SIZE/2.0)
...
def save_data(idx_data=1, train=True, n_control_points=8, k_size=64):
	F_pixels = 1
	k_size = k_size
	n_control_points = n_control_points
	grid_in = get_grid_image(k_size, k_size//2, 1)
	...
	vec_list = np.zeros([LEN, 2*(n_control_points-1)])
	k_list = np.zeros([LEN, k_size, k_size])
	SEED_IDX = idx_data
	if train:
		SEED_IDX += 20
	np.random.seed((2*SEED_IDX+1)**2)
	for idx in  tqdm(range(LEN)):		
		control_points = get_control_points(n_control_points)
		trajectory = create_trajectory(control_points, 500)
		kernel = center_kernel(convert_trajectory_to_kernels(trajectory, grid_in, K_list))
		control_points_vec = np.zeros([2*(n_control_points-1)], dtype=np.float32)
		# Convert control_points to a standardised vector format
		# Convert (x,y,1/0, 1/0) coordinates to 4 X (self.n_control_points-1) X 1 vector
		# By convention, first point is (0,0) so it is dropped
		N1 = np.shape(control_points)[0]
		for ii in range(1,np.shape(control_points)[0]):
			control_points_vec[(ii-1)*2] = control_points[ii,0]
			control_points_vec[(ii-1)*2+1] = control_points[ii,1]
			
		vec_list[idx,:] = control_points_vec
		k_list[idx,:,:] = kernel
	if train:
		np.savez('kernel_data_train_'+str(idx_data)+'_'+str(n_control_points)+'.npz',vec_list=vec_list,k_list=k_list)
	else:
		np.savez('kernel_data_val_'+str(idx_data)+'_'+str(n_control_points)+'.npz',vec_list=vec_list,k_list=k_list)

if __name__ == "__main__":

	N_CONTROL = 8
	for idx in range(10):
		save_data(idx+1, True, N_CONTROL)
	for idx in range(5):
		save_data(idx+1, False, N_CONTROL)
