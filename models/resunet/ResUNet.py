...
class ResUNet_Kernel(nn.Module):
	def __init__(self, in_nc=1, out_nc=1, nc=[16, 16, 32, 32], nb=2, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose'):
		super(ResUNet_Kernel, self).__init__()
		self.layer_norm = nn.LayerNorm([1,32,32])
		self.m_head = B.conv(in_nc, nc[0], bias=False, mode='C')
		# downsample
		downsample_block = B.downsample_strideconv	
		self.m_down1 = B.sequential(*[B.ResBlock(nc[0], nc[0], bias=False, mode='C'+act_mode+'C') for _ in range(nb)], downsample_block(nc[0], nc[1], bias=False, mode='2'))
